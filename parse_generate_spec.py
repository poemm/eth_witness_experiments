...
def parse_Nibbles(bytes_, idx, nibbleslen):
  if verbose: print("parse_Nibbles",bytes_,idx,nibbleslen)
...
def parse_Tree(bytes_,idx):
  ...
  idx,w = parse_Tree_Node(bytes_,idx,0)
  return idx, (c,w)
...
def parse_Tree_Node(bytes_, idx, d):
  assert d<65
  idx, nodetype = parse_Byte(bytes_, idx)
  assert nodetype in {0x00,0x01,0x02,0x03}
  if nodetype==0x00:
    idx, branch = parse_Branch_Node(bytes_, idx, d)
    return idx, branch
  elif nodetype==0x01:
    idx, extension = parse_Extension_Node(bytes_, idx, d)
    return idx, extension
  elif nodetype==0x02:
    idx, account = parse_Account_Node(bytes_, idx, d)
    return idx, account
  elif nodetype==0x03:
...
def parse_Branch_Node(bytes_, idx, depth):
  if verbose: print("parse_Branch_Node",idx,depth)
  assert depth < 64
  ...
  for i in range(16):
    if bitmaskstr[i]=='1':
      idx, child = parse_Tree_Node(bytes_, idx, depth+1)
      c[i] = child
  return idx, ("branch", c)
...
def parse_Extension_Node(bytes_, idx, depth):
  assert depth < 63
  idx, nibbleslen = parse_Byte(bytes_, idx)
  idx, nibbles = parse_Nibbles(bytes_, idx, nibbleslen)
  idx, child = parse_Child_Of_Extension_Node(bytes_, idx, depth+nibbleslen)
...
def parse_Child_Of_Extension_Node(bytes_, idx, depth):
  if verbose: print("parse_Child_Of_Extension_Node",bytes_,idx,depth)
  assert depth<65
  idx, nodetype = parse_Byte(bytes_, idx)
  assert nodetype in {0x00,0x03}
  if nodetype==0x00:
    idx, branch = parse_Branch_Node(bytes_, idx, depth)
    return idx, branch
  elif nodetype==0x03:
    idx, hash_ = parse_Bytes32(bytes_, idx)
    return idx, hash_
...
def parse_Account_Node(bytes_,idx, depth):
  ...
  if accounttype == 0x01:
    ...
    idx, storage = parse_Account_Storage_Tree_Node(bytes_,idx,0)
...
def gen_Account_Node(file_, account):
  if verbose: print("gen_Account_Node",file_,account)
  if len(account)==5:
    gen_Byte(file_, 0x00)
  elif len(account)==7:
    gen_Byte(file_, 0x01)
  gen_Nibbles(file_,account[1][1])
  gen_Address(file_,account[2])
  gen_Bytes32(file_,account[3])
  gen_Bytes32(file_,account[4])
  if len(account)>5:
    gen_Bytecode(file_,account[5])
    gen_Account_Storage_Tree_Node(file_,account[6])
...
############################
# account storage tree nodes

def parse_Account_Storage_Tree_Node(bytes_, idx, d):
  assert d<65
  idx, nodetype = parse_Byte(bytes_, idx)
  assert nodetype in {0x00,0x01,0x02,0x03}
  if nodetype==0x00:
    idx, branch = parse_Account_Storage_Branch_Node(bytes_, idx, d)
    return idx, branch
  elif nodetype==0x01:
    idx, extension = parse_Account_Storage_Extension_Node(bytes_, idx, d)
    return idx, extension
  elif nodetype==0x02:
    idx, account = parse_Account_Storage_Account_Node(bytes_, idx, d)
    return idx, account
  elif nodetype==0x03:
    idx, hash_ = parse_Bytes32(bytes_, idx)
    return idx, hash_

def gen_Account_Storage_Tree_Node(file_, node):
  if verbose: print("gen_Tree_Node",file_,node)
  if node[0]=="branch":
    gen_Byte(file_, 0x00)
    gen_Account_Storage_Branch_Node(file_, node)
  elif node[0]=="extension":
    gen_Byte(file_, 0x01)
    gen_Account_Storage_Extension_Node(file_, node)
  elif node[0]=="account":
    gen_Byte(file_, 0x02)
    gen_Account_Storage_Leaf_Node(file_, node)
  elif node[0]=="hash":
    gen_Byte(file_, 0x03)
    gen_Bytes32(file_, node[1])
  else:
...
def parse_Account_Storage_Branch_Node(bytes_, idx, depth):
  assert depth < 64
  idx, bitmask = parse_Bytes2_More_Than_One_Bit_Set(bytes_, idx)
  bitmaskstr = bin(bitmask[0])[2:].zfill(8) + bin(bitmask[1])[2:].zfill(8)
  c = [None]*16
  for i in range(16):
    if bitmaskstr[i]=='1':
      idx, child = parse_Account_Storage_Tree_Node(bytes_, idx, depth)
      c[i] = child
  return idx, ("branch", c)

def gen_Account_Storage_Branch_Node(file_,b):
  if verbose: print("gen_Account_Storage_Branch_Node",file_,b)
  # generate bitmask
  bitmaskstr = ""
  for branch in b[1:]:
    if branch:
  ...
  for i in range(16):
    if bitmaskstr[i]=='1':
      gen_Account_Storage_Tree_Node(file_,b[i+1])

def parse_Account_Storage_Extension_Node(bytes_, idx, depth):
  assert depth < 63
  idx, nibbleslen = parse_Byte(bytes_, idx)
  idx, nibbles = parse_Nibbles(bytes_, idx, nibbleslen)
  idx, child = parse_Child_Of_Account_Storage_Extension_Node(bytes_, idx, depth+nibbleslen)
  # if odd number of nibbles, last one must be zeros
  return idx, ("extension", (nibbleslen, nibbles), child)

def gen_Account_Storage_Extension_Node(file_,e):
  if verbose: print("gen_Extension_Node",file_,e)
  gen_Byte(file_, e[1][0])
  gen_Nibbles(file_, e[1][1])
  gen_Child_Of_Account_Storage_Extension_Node(file_,e[2])

def parse_Child_Of_Account_Storage_Extension_Node(bytes_, idx, depth):
  if verbose: print("parse_Child_Of_Extension_Node",bytes_,idx,depth)
  assert depth<65
  idx, nodetype = parse_Byte(bytes_, idx)
  assert nodetype in {0x00,0x03}
  if nodetype==0x00:
    idx, branch = parse_Account_Storage_Branch_Node(bytes_, idx, depth)
    return idx, branch
  elif nodetype==0x03:
    idx, hash_ = parse_Bytes32(bytes_, idx)
    return idx, hash_
...
def parse_Account_Storage_Leaf_Node(bytes_,idx, depth):
  assert depth<65
  idx, accounttype = parse_Byte(bytes_, idx)
  assert accounttype in {0x00,0x01}
  idx, pathnibbles = parse_Nibbles(bytes_,idx,64-depth)
...
def gen_Account_Storage_Leaf_Node(file_, account):
  if verbose: print("gen_Account_Storage_Leaf_Node",file_,account)
  gen_Nibbles(file_,account[1])
  gen_Bytes32(file_,account[2])
  gen_Bytes32(file_,account[3])
  if len(account)>4:
    gen_Bytecode(file_,account[4])
    gen_Account_Storage_Tree_Node(file_,account[5])
